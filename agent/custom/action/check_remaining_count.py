...
COUNT_PATTERN = re.compile(
    r"(?<![0-9A-Za-z])([0-9oO](?:\s*[0-9oO])*)\s*/\s*([0-9oO](?:\s*[0-9oO])*)"
)
OCR_DIGIT_TRANSLATION = str.maketrans({"o": "0", "O": "0"})
...
def _normalize_text(text: str) -> str:
    ...
    return normalized.translate(SLASH_TRANSLATION)


def _normalize_digit_token(token: str) -> str:
    return re.sub(r"\s+", "", token).translate(OCR_DIGIT_TRANSLATION)


def _parse_left_count(text: str) -> int | None:
    normalized = _normalize_text(text)
    match = COUNT_PATTERN.search(normalized)
    if not match:
        return None

    left_token = _normalize_digit_token(match.group(1))
    if not left_token or not left_token.isdigit():
        return None

    return int(left_token)
...
@AgentServer.custom_action("CheckRemainingCount")
class CheckRemainingCount(CustomAction):
    def run(
        self,
        context: Context,
        ...
        for text in _iter_ocr_texts(argv.reco_detail):
            ...
            result = left_count > 20
            print(
                f"CheckRemainingCount text={text!r}, left_count={left_count}, result={result}"
            )
            return result

        print("CheckRemainingCount found no valid count text.")
        return False
