...
# ---------- 新动作：判断混沌遗迹剩余次数是否大于 0 ----------
@AgentServer.custom_action("CheckChaosRelicCount")   # ← 新名字
class CheckChaosRelicCount(CustomAction):            # ← 新类名
    def run(
        self,
        context: Context,
        ...
        for text in _iter_ocr_texts(argv.reco_detail):
            ...
            print(f"CheckChaosRelicCount checking text: {text!r}, parsed left_count: {left_count}")
            if left_count is None:
                continue

            # 剩余次数 > 0 返回 True，等于 0 返回 False
            result = left_count > 0
            print(
        ...
        print("CheckChaosRelicCount found no valid count text.")
        return False