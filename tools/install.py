...
def ignore_wrong_resource_model(src, names):
    if Path(src).resolve() == (working_dir / "assets" / "resource").resolve():
        return {"model"} if "model" in names else set()
    return set()
...
def install_resource():
    ...
    wrong_model_dir = install_path / "resource" / "model"
    if wrong_model_dir.exists():
        shutil.rmtree(wrong_model_dir)

    shutil.copytree(
        working_dir / "assets" / "resource",
        install_path / "resource",
        ignore=ignore_wrong_resource_model,
        dirs_exist_ok=True,
    )
    shutil.copy2(
